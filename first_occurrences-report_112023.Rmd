...
met_long2 <- dat_phase1 %>%
  dplyr::select(eid, age_cat, race, smoker = current_tobacco_smoking_f1239_0_0, menopause_cat, sex = sex_f31_0_0, all_of(metab_key2$col.name)) %>%
  pivot_longer(-c(eid, age_cat, race, smoker, menopause_cat, sex), names_to = "metabolite", values_to = "value")

long_test_3 <- met_long2 %>%
  filter(age_cat %in% c("<50", "50-59")) %>%
  drop_na()%>% 
  nest(.by = c(age_cat, metabolite)) %>%
  mutate(model2 = map(data, ~lm(value ~ sex + menopause_cat + smoker + race, data = .x)),
...
long_test_4 <- met_long2 %>%
  ...
  mutate(model2 = map(data, ~lm(value ~ sex + smoker + race, data = .x,
...
# volcano plot
ggplot(res_overall, aes(x = estimate.x, y = -log10(fdr.y+1), color = sig_category_overall, alpha = 0.1)) +
  geom_point() +
  scale_color_manual(values = c("gray", "black", "blue"), name = "")+
  facet_wrap(~age_cat) +
  guides(alpha = FALSE) +
...
res_overall %>%
  tbl_cross(row = age_cat, col = sig_category_overall)
...
met_long3 <- dat_sub_metab2 %>%
  filter(has_metabolites_phase1 == TRUE | has_metabolites_phase2 == TRUE) %>%
  # doing center scaling so we can compare estimates?
  mutate(across(
    all_of(metab_key2$col.name),scale)
  ) %>%
...
# 
# save(long_med1, file = "mediation_u50.Rdata")
...
# save(long_med2, file = "mediation_5059.Rdata")

load(file = "mediation_5059.Rdata")
...
# save(long_med3, file = "mediation_6069.Rdata")
...
# save(long_med4, file = "mediation_7085.Rdata") # 7 min?
load(file = "mediation_7085.Rdata")

med_res_85 <- long_med4 %>%dplyr::select(metabolite, age_cat, tidied_m) %>%
  unnest(tidied_m)# %>%
  #filter(term == "sex.L")

med_all_strat <- bind_rows(med_res_lt50, med_res_lt59, med_res_lt69, med_res_85) %>%
  mutate(fdr = p.adjust(p.value, "fdr"),
         phase = "Phase I & 2",
         met_threshold = fdr < 0.05 & abs(estimate)>0.5)
