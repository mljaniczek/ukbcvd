...
res_lt50_to_59_p2 <- long_test_phase2 %>% dplyr::select(metabolite, age_cat, tidied) %>%
  unnest(tidied) %>%
  filter(term == "sex.L")
...
res_60_to_85_p2 <- long_test_2_phase2 %>% dplyr::select(metabolite, age_cat, tidied) %>%
  unnest(tidied) %>%
  filter(term == "sex.L")
...
met_long2 <- dat_phase1 %>%
  dplyr::select(eid, age_cat, race, smoker = current_tobacco_smoking_f1239_0_0,
                bmi = body_mass_index_bmi_f21001_0_0, menopause_cat, sex = sex_f31_0_0, all_of(metab_key2$col.name)) %>%
  pivot_longer(-c(eid, age_cat, race, smoker, bmi, menopause_cat, sex), names_to = "metabolite", values_to = "value")
...
long_test_3 <- met_long2 %>%
  ...
  mutate(model_y = map(data, ~lm(value ~ sex + menopause_cat + bmi + smoker + race, data = .x)))

long_test_3 <- long_test_3 %>%
  mutate(tidied_y = map(model_y, ~tidy(.x, conf.int = T)))

long_test_3 <- long_test_3 %>%
  mutate(
         model_m = map(data, ~lm(bmi ~ sex + menopause_cat + smoker + race, data = .x)))
...
long_test_4 <- met_long2 %>%
  ...
  nest(.by = c(age_cat, metabolite)) %>%
  # set up both outcome model (model_y) and mediator model (model_m)
  mutate(model_y = map(data, ~lm(value ~ sex + bmi + smoker + race, data = .x,
                                 x = FALSE, y = FALSE, model = FALSE)),
         model_m = map(data, ~lm(bmi ~ sex + smoker + race, data = .x)))%>%
        mutate(tidied_y = map(model_y, ~tidy(.x, conf.int = T))) 
...
res_lt50_to_59_M2 <- long_test_3 %>% dplyr::select(metabolite, age_cat, tidied_y) %>%
  unnest(tidied_y) %>%
  filter(term == "sex.L")

# med_res_lt50_to_59_M2 <- long_test_3 %>% dplyr::select(metabolite, age_cat, tidied_m) %>%
#   unnest(tidied_m)# %>%
  #filter(term == "sex.L")



res_60_to_85_M2 <- long_test_4 %>% dplyr::select(metabolite, age_cat, tidied_y) %>%
  unnest(tidied_y) %>%
  filter(term == "sex.L")

# long_test_4 <- long_test_4 %>%
#   mutate(
#          mediation_res = map2(model_m, model_y, 
#                               ~mediate(model.m = .x, model.y = .y,
#                                        boot = TRUE, 
#                                        treat = "sex",
#                                        mediator = "bmi")),
#          tidied_m = map(mediation_res, ~tidy(.x, conf.int = T))) 

# med_res_60_to_85_M2 <- long_test_4 %>%dplyr::select(metabolite, age_cat, tidied_m) %>%
#   unnest(tidied_m) #%>%
  #filter(term == "sex.L")
...
# med_all_strat_M2 <- bind_rows(med_res_lt50_to_59_M2, med_res_60_to_85_M2) %>%
#   mutate(fdr = p.adjust(p.value, "fdr"),
#          phase = "Phase I",
#          met_threshold = fdr < 0.05 & abs(estimate)>0.5) 
...
long_test_3_p2 <- long_test_3_p2 %>%
  mutate(tidied_y = map(model_y, ~tidy(.x, conf.int = T))) %>%
  mutate(model_m = map(data, ~lm(bmi ~ sex + menopause_cat + smoker + race, data = .x)))
...
res_lt50_to_59_M2_p2 <- long_test_3_p2 %>% dplyr::select(metabolite, age_cat, tidied_y) %>%
  unnest(tidied_y) %>%
  filter(term == "sex.L")
...
res_60_to_85_M2_p2 <- long_test_4_p2 %>% dplyr::select(metabolite, age_cat, tidied_y) %>%
  unnest(tidied_y) %>%
  filter(term == "sex.L")
...
all_strat_both_phases_wide_M2 <- left_join(
  ...
  mutate(sig_category_m2 = ifelse(
    ...
    ifelse( met_threshold_m2 == TRUE & met_threshold_m2_p2 == FALSE, "Met threshold, not validated",
            ifelse(
              met_threshold_m2 == TRUE & met_threshold_m2_p2 == TRUE, "Validated in Phase 2", "Didn't meet threshold"
            ))
  ))
...
all_strat_both_phases_long_m2 <- bind_rows(res_M2, all_strat_M2_p2) %>%
  dplyr::select(metabolite, age_cat, phase, sig_category_m2, estimate, fdr, met_threshold_m2_p1 = met_threshold_m2, met_threshold_m2_p2)
...
# volcano plot
ggplot(res_M2, aes(x = estimate, y = -log10(fdr+1), color = sig_category_m2, alpha = 0.1)) +
  geom_point() +
  scale_color_manual(values = c("gray", "black", "blue"), name = "")+
  facet_wrap(~age_cat) +
  guides(alpha = FALSE) +
...
res_M2 %>%
  tbl_cross(row = age_cat, col = sig_category_m2)
...
validated_mets_m2 <- res_M2 %>%
  filter(sig_category_m2 == "Validated in Phase 2")
```
...
```
 ...
 number of unique validated metabolites where the metabolite met the thresholds in Model 2 both phase 1 and phase 2. Saved table of these 57 unique metabolites and their metabolite group and subgroup (validated_unique_mets_m2_57_details.csv). 
...
res_overall <- res_M1 %>%
  left_join(res_M2, by = c("metabolite", "age_cat")) %>%
    ...
    sig_category == "Validated in Phase 2" & sig_category_m2 == "Validated in Phase 2", "Validated in P2 and M2",
    ifelse( sig_category == "Validated in Phase 2" & sig_category_m2 != "Validated in Phase 2", "Validated in P2, not in M2", "Not validated/Didn't meet threshold")))


# volcano plot
ggplot(res_overall, aes(x = estimate.y, y = -log10(fdr.y+1), color = sig_category_overall, alpha = 0.1)) +
  geom_point() +
  scale_color_manual(values = c("gray", "black", "blue"), name = "")+
  facet_wrap(~age_cat) +
  guides(alpha = FALSE) +
